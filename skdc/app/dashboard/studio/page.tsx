...
/**
 * Top-level entry to the Studio.
 *
 * - 0 projects → CTA to create one (we can't open a studio without a project).
 * - 1 project  → straight to that project's studio.
 * - many      → picker grid.
 */
export default async function StudioEntryPage({
  ...
  if (projects.length === 1 && !pick) {
    redirect(`/dashboard/projects/${projects[0].id}/studio`);
  }
      ...
      <div>
        ...
        <div className="glass relative overflow-hidden rounded-3xl p-10 text-center">
          ...
          <p className="relative mx-auto mt-3 max-w-md text-sm leading-relaxed text-muted-foreground">
            الاستديو يفتح داخل أي مشروع. أنشئ مشروعك الأول وستعود لهنا للتصميم.
          </p>
